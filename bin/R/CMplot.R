    ...
    if(multracks==FALSE){
      ...
      for(i in 1:R){
        ...
        if(is.null(ylim)){
          ...
          if(Max<=1){
            if(cir.density){
              plot(pvalue.posN,logpvalue,pch=pch,cex=cex[2],col=rep(rep(colx,N[i]),add[[i]]),ylab="",
                   xlim=c(0,1.01*max(pvalue.posN)),ylim=c(-Max/den.fold, Max+10^(-ceiling(-log10(Max)))),
                   cex.axis=cex.axis,cex.lab=cex.lab,font=2,axes=FALSE,xlab=xlab,main=main)
              #title(ylab=ylab, line=line, cex.lab=cex.lab, family="Calibri Light")
            }else{
              plot(pvalue.posN,logpvalue,pch=pch,cex=cex[2],col=rep(rep(colx,N[i]),add[[i]]),
                   xlim=c(0,max(pvalue.posN)),ylim=c(0,Max+10^(-ceiling(-log10(Max)))),ylab="",
                   cex.axis=cex.axis,cex.lab=cex.lab,font=2,axes=FALSE,xlab=xlab,type=type,main=main)
              #title(ylab=ylab, line=line, cex.lab=cex.lab, family="Calibri Light")
            }
          }else{
            if(cir.density){
              plot(pvalue.posN,logpvalue,pch=pch,cex=cex[2],col=rep(rep(colx,N[i]),add[[i]]),
                   xlim=c(0,1.01*max(pvalue.posN)),ylim=c(-Max/den.fold,Max+1),ylab="",
                   cex.axis=cex.axis,cex.lab=cex.lab,font=2,axes=FALSE,xlab=xlab,main=main)
              #title(ylab=ylab, line=line, cex.lab=cex.lab, family="Calibri Light")
            }else{
              plot(pvalue.posN,logpvalue,pch=pch,cex=cex[2],col=rep(rep(colx,N[i]),add[[i]]),
                   xlim=c(0,max(pvalue.posN)),ylim=c(0,Max+1),ylab="",
                   cex.axis=cex.axis,cex.lab=cex.lab,font=2,axes=FALSE,xlab=xlab,type=type,main=main)
              #title(ylab=ylab, line=line, cex.lab=cex.lab, family="Calibri Light")
            }
          }
          ...
          Max <- max(ylim)
          if(cir.density){
            plot(pvalue.posN[logpvalue>=min(ylim)],logpvalue[logpvalue>=min(ylim)],pch=pch,cex=cex[2],col=rep(rep(colx,N[i]),add[[i]])[logpvalue>=min(ylim)],
                 xlim=c(0,1.01*max(pvalue.posN)),ylim=c(min(ylim)-Max/den.fold, max(ylim)), ylab="",
                 cex.axis=cex.axis,cex.lab=cex.lab,font=2,axes=FALSE,xlab=xlab,main=main)
            #title(ylab=ylab, line=line, cex.lab=cex.lab, family="Calibri Light")
          }else{
            plot(pvalue.posN[logpvalue>=min(ylim)],logpvalue[logpvalue>=min(ylim)],pch=pch,cex=cex[2],col=rep(rep(colx,N[i]),add[[i]])[logpvalue>=min(ylim)],
                 xlim=c(0,max(pvalue.posN)),ylim=ylim, ylab="",
                 cex.axis=cex.axis,cex.lab=cex.lab,font=2,axes=FALSE,xlab=xlab,type=type,main=main)
            #title(ylab=ylab, line=line, cex.lab=cex.lab, family="Calibri Light")
          }
        }
        ...
        if(!is.null(threshold)){
	      ...
	      #legend(2*1000000,threLE,legend=legend,col=threshold.col[thr], lty=threshold.lty[thr],cex=1.2,box.lty=0,)
        ...
        if(Max<=1){
          if(cir.density){
            plot(NULL,xlim=c(0,1.01*max(pvalue.posN)),ylim=c(-Max/den.fold, Max+10^(-ceiling(-log10(Max)))),ylab=ylab,
                 cex.axis=cex.axis,cex.lab=2,font=2,axes=FALSE,xlab=xlab)
          }else{
            plot(NULL,xlim=c(0,max(pvalue.posN)),ylim=c(0,Max+10^(-ceiling(-log10(Max)))),ylab=ylab,
                 cex.axis=cex.axis,cex.lab=2,font=2,axes=FALSE,xlab=xlab,type=type)
          }
        }else{
          if(cir.density){
      ...
      if(is.null(chr.labels)){
        axis(1, at=c(0,ticks),cex.axis=cex.axis,font=2,labels=c(chr.ori))
      }else{
        axis(1, at=c(0,ticks),cex.axis=cex.axis,font=2,labels=c("Chr",chr.labels))
      }
      if(is.null(ylim)){
